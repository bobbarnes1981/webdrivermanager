...
namespace WebDriverManagerSharp.UnitTests.Managers
{
    ...
    public class InternetExplorerDriverManagerTests : BaseManagerTests
    {
        ...
        {
            WebDriverManager.IEDriver().Version("latest");

            configMock.Verify(x => x.SetInternetExplorerDriverVersion("latest"), Times.Once);
        }
        ...
        public void SetDriverRepositoryUrl()
        {
            ...
            WebDriverManager.IEDriver().DriverRepositoryUrl(uri);

            configMock.Verify(x => x.SetInternetExplorerDriverUrl(uri), Times.Once);
        }
        ...
        public void TestSetUpVersion()
        {
            ...
            fileStorageMock.Setup(x => x.GetCurrentDirectory()).Returns("");

            Uri props = new Uri("http://props");
            configMock.Setup(x => x.GetVersionsPropertiesUrl()).Returns(props);
            httpClientMock.Setup(x => x.ExecuteHttpGet(props, null)).Returns(new MemoryStream(Encoding.ASCII.GetBytes("\r\n")));
        ...
        [Test]
        public void TestSetUpVersionPropertiesError()
        {
            Uri driverUrl = new Uri("https://fake.selenium-release.storage.googleapis.com/");
            configMock.Setup(x => x.GetInternetExplorerDriverUrl()).Returns(driverUrl);
            ...
            WebDriverManagerException exception = Assert.Throws<WebDriverManagerException>(() => WebDriverManager.IEDriver().Setup());

            Assert.That(exception.InnerException, Is.InstanceOf<IllegalStateException>());
        }
